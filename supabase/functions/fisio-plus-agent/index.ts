...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Obtendo prompt do agente Fisio+");
...
Responda com um objeto JSON seguindo este formato:
{
  ...
  "days": {
    ...
    // Call Groq API with robust error handling and timeout
    ...
    try {
      ...
      console.log("Conteúdo recebido do modelo. Processando...");
      ...
      // Validate the rehabilitation plan structure
      if (!rehabPlan.days || !rehabPlan.overview) {
        console.error("Plano de reabilitação inválido - faltam campos obrigatórios:", rehabPlan);
        throw new Error("O plano de reabilitação gerado está incompleto. Por favor, tente novamente.");
      }

      // Save the rehabilitation plan to the database if user is authenticated
      if (userId) {
        console.log("Salvando plano de reabilitação para o usuário:", userId);
        const supabase = supabaseClient();
      ...
      if (fetchError.name === 'AbortError') {
        console.error("Timeout ao chamar a API do Groq");
        throw new Error("Tempo limite excedido ao gerar o plano. Por favor, tente novamente.");
      }
      throw fetchError;
    }
  } catch (error) {
