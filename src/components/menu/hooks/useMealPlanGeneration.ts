...
export const generateMealPlan = async ({
  ...
  try {
    ...
    if (userData.id) {
      try {
        console.log("💾 Tentando salvar plano alimentar no banco de dados para o usuário:", userData.id);
        const { data: savePlanData, error: saveError } = await supabase
          .from('meal_plans')
          .insert({
            user_id: userData.id,
        ...
        if (saveError) {
          ...
          toast.error("Erro ao salvar o plano alimentar, mas você ainda pode visualizá-lo.");
        } else {
        ...
        toast.error("Erro ao salvar o plano alimentar no banco de dados.");
      }
    } else {
      console.warn("⚠️ Usuário não autenticado, plano não será salvo no banco de dados");
    }
